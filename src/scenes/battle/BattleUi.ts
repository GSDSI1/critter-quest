...
import { drawHpBar, paintHpBar } from '../../ui/HUD';
...
export class BattleUi {
  ...
  private playerHpTween?: Phaser.Tweens.Tween;
  ...
  build(): void {
    ...
    this.scene.add.ellipse(480, 168, 72, 14, 0x000000, 0.22).setDepth(1);
    this.enemySprite = addCreatureImage(this.scene, 480, 130, this.host.wild.speciesId).setScale(1.5).setDepth(2);
    this.enemyIdle = startCritterIdle(this.scene, this.enemySprite, this.host.wild.speciesId, 130);

    buildMenuPanel(this.scene, 340, 24, 260, 80, 0);

    this.enemyNameText = this.scene.add.text(356, 32, '', titleStyle('14px'));
    this.enemyHpBar = drawHpBar(this.scene, 356, 68, 180, 10, 0, 1);

    this.scene.add.image(180, 340, 'battle_platform').setAlpha(0.9);
    this.scene.add.ellipse(160, 332, 88, 16, 0x000000, 0.22).setDepth(1);
    this.playerSprite = addCreatureImage(this.scene, 160, 290, this.host.playerMon.speciesId, false, 'back').setScale(2).setFlipX(true).setDepth(2);
    ...
    buildMenuPanel(this.scene, 40, 248, 260, 100, 0);

    this.scene.add.text(56, 256, displayName(this.host.playerMon), titleStyle('14px'));
    this.scene.add.text(56, 274, `Lv.${this.host.playerMon.level}`, hintStyle('11px'));
    this.playerHpText = this.scene.add.text(200, 274, '', hintStyle('10px')).setOrigin(1, 0);
    this.abilityText = this.scene.add.text(56, 288, '', hintStyle('10px'));
    this.playerHpBar = drawHpBar(this.scene, 56, 300, 180, 10, 0, 1);
    this.expBar = drawHpBar(this.scene, 56, 316, 180, 6, 0, 1, 50);
    this.playerStatsText = this.scene.add.text(56, 330, '', hintStyle('10px'));
  ...
  refreshPlayerUi(): void {
    ...
    paintHpBar(this.expBar, 56, 316, 180, 6, prog * 180, 180, 50);
  ...
  private animateHp(
    ...
    const tweenRef = isEnemy ? 'enemyHpTween' : 'playerHpTween';
    this[tweenRef]?.stop();
    this[tweenRef] = undefined;

    const key = isEnemy ? 'enemyDisplayedHp' : 'playerDisplayedHp';
    const start = this[key];
    if (start === targetCurrent) {
      paintHpBar(bar, x, y, 180, 10, targetCurrent, max);
      if (!isEnemy) {
        this.playerHpText.setText(`${targetCurrent}/${max} ${statusLabel(this.host.playerMon.status)}`);
      }
      return;
    }

    this[tweenRef] = this.scene.tweens.addCounter({
      from: start,
      to: targetCurrent,
      duration: 300,
      ease: 'Quad.easeOut',
      onUpdate: tween => {
        ...
        this[key] = val;
        paintHpBar(bar, x, y, 180, 10, val, max);
        if (!isEnemy) {
          this.playerHpText.setText(`${val}/${max} ${statusLabel(this.host.playerMon.status)}`);
        }
      },
      onComplete: () => {
        this[key] = targetCurrent;
        paintHpBar(bar, x, y, 180, 10, targetCurrent, max);
        if (!isEnemy) {
          this.playerHpText.setText(`${targetCurrent}/${max} ${statusLabel(this.host.playerMon.status)}`);
        }
        this[tweenRef] = undefined;
        if (targetCurrent / max < 0.25 && targetCurrent > 0) {
